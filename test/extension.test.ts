...
-1 -999999999999999999999999999999999 -1e6 -1e-6 -999999999999999.999999999999999999 -1. -.1 -1.e6 -1.e-6 -.1e6 -.1e-6 `;
    ...
    test('Lex Floats', () => {
      ...
      assert.deepStrictEqual(lexer.next(), { type: TokenType.Float, value: 1.0 });
      assert.deepStrictEqual(lexer.next(), { type: TokenType.Float, value: 0.1 });
    });
    ...
    test('Lex Floats with Exponents', () => {
      ...
      assert.deepStrictEqual(lexer.next(), { type: TokenType.Float, value: 0.1e6 });
      assert.deepStrictEqual(lexer.next(), { type: TokenType.Float, value: 0.1e-6 });
    });
