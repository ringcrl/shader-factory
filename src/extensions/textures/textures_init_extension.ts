...
import { TextureExtensionExtension } from './texture_extension_extension';
    ...
    for (const i in buffers) {
      ...
      for (const texture of textures) {
        ...
        if (texture.Type !== undefined && texture.Type === Types.TextureType.CubeMap) {
          ...
          const getTexturesFromPrefixes = (pattern: string, prefixes: [ string, string, string, string, string, string ]) => {
            const textures = [];
          ...
          textures = textures.map((texture: string) => makeAvailableResource(texture));
