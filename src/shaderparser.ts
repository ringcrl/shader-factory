...
import {
  ShaderLexer, Token, TokenType, LineRange,
} from './shaderlexer';
...
export class ShaderParser {
  ...
  private testAssignable(leftType: string, rightType: string) {
    ...
    const [rightMappedType, rightIsVecType] = this.mapVecTypesToArrayTypes(rightType);
        ...
        const arraySizesMatch = leftTypeDesc.ArraySizes.every((arraySize: number, index: number) => arraySize == 0 || arraySize == rightTypeDesc.ArraySizes[0]);
