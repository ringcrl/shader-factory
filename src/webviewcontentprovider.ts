...
import { ResolutionExtension } from './extensions/user_interface/resolution_extension';
import { ResolutionStyleExtension } from './extensions/user_interface/resolution_style_extension';
import { ExportExtension } from './extensions/user_interface/export_extension';
import { ExportStyleExtension } from './extensions/user_interface/export_style_extension';
...
export class WebviewContentProvider {
  ...
  public parseShaderTree(generateStandalone: boolean): string[] {
    ...
    // Parse Shaders
    {
      ...
      new BufferProvider(this.context)
        .parseShaderCode(shaderName, shader, this.buffers, this.commonIncludes, generateStandalone);
    ...
    localResources = localResources.filter((elem, index, self) => index === self.indexOf(elem));
  ...
  public generateWebviewContent(webview: vscode.Webview | undefined, startingState: Types.RenderStartingData): string {
    ...
    const resolutionStyleExtension = new ResolutionStyleExtension();
    this.webviewAssembler.addWebviewModule(resolutionStyleExtension, '/* Resolution Button Style */');
    const resolutionExtension = new ResolutionExtension(JSON.stringify(forcedAspect));
    this.webviewAssembler.addWebviewModule(resolutionExtension, '<!-- Resolution Button -->');

    const exportStyleExtension = new ExportStyleExtension();
    this.webviewAssembler.addWebviewModule(exportStyleExtension, '/* Export Button Style */');
    const exportExtension = new ExportExtension();
    this.webviewAssembler.addWebviewModule(exportExtension, '<!-- Export Button -->');
    ...
    // Texture Loading
    const textureInitExtension = new TexturesInitExtension(this.buffers, this.context, makeWebviewResource);
