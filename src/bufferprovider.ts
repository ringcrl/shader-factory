...
export class BufferProvider {
  ...
  private makeName(path: string): string {
    ...
    const trim = (name: string) => name.replace(/^["]+|["]+$/g, '');
    return trim(name);
  }
  ...
  private parseShaderCodeInternal(rootFile: string, file: string, code: string, buffers: Types.BufferDefinition[], commonIncludes: Types.IncludeDefinition[], generateStandalone: boolean) {
...

    {
      ...
      if (versionPos === 0) {
        ...
        this.showInformationAtLine(file, `Version directive '${versionDirective}' ignored by shader-factory extension`, 0);
      }
    }
...
void main() {
    ...
    mainImage(gl_FragColor, fragCoord);
      ...
      if (this.context.getConfig<boolean>('shaderToyStrictCompatibility') || strictComp.Value) {
        ...
        // If there is no void main() in the shader we assume it is a shader-factory style shader
    ...
    while (!parser.eof()) {
      ...
      switch (nextObject.Type) {
        ...
        case ObjectType.Texture: {
          ...
          if (local) {
            ...
            if (userPath === 'self') {
              ...
              ({ file: textureFile, userPath } = this.context.mapUserPath(userPath, file));
        ...
        case ObjectType.Include: {
          ...
          ({ file: includeFile, userPath } = this.context.mapUserPath(userPath, file));
          ...
          if (sharedIncludeIndex < 0) {
            const includeCode = this.readShaderFile(includeFile);
            if (includeCode.success) {
              const include_line_offset: Types.BoxedValue<number> = { Value: 0 };
              const transformedIncludeCode = this.transformCode(rootFile, includeFile, includeCode.bufferCode, include_line_offset, textures, textureSettings,
